...
const Dashboard: React.FC = () => {
	...
	const [aggregatedStats, setAggregatedStats] = useState<
		Record<string, AggregatedSensorData>
	>({});
	...
	useEffect(() => {
		...
		const connectToHub = async () => {
			try {
				...
				if (state && (state as any) === 1) {
					// Already Connected
					console.log("SignalR already connected");
					setIsConnected(true);
					return;
				}
				if (state && (state as any) === 0) {
					// Connecting
				...
				data.aggregated.forEach((stat) => {
					...
					if (currentCount > lastCount) {
						...
						const alert: AnomalyAlert = {
							...
							alertType: alertType,
							message: `${currentCount - lastCount} new anomal${
								currentCount - lastCount === 1 ? "y" : "ies"
							} detected: ${stat.type} sensors ${threshold.message}`,
				...
				data.aggregated.forEach((stat) => {
					...
					if (sensorType in newChartData) {
						...
						if (newChartData[sensorType].length > ROLLING_WINDOW_SIZE) {
							newChartData[sensorType] = newChartData[sensorType].slice(
								-ROLLING_WINDOW_SIZE
							);
						}
					}
