...
const API_BASE_URL =
	import.meta.env.VITE_API_BASE_URL || "http://localhost:5135";
...
export class SensorApiService {
	...
	public async getAllStatistics(): Promise<ApiStatsResponse> {
		return this.fetchJson<ApiStatsResponse>(
			`${API_BASE_URL}/api/sensors/stats`
		);
	}
	...
	public async getRecentReadings(
		count: number = 100
	): Promise<ApiRecentResponse> {
		return this.fetchJson<ApiRecentResponse>(
			`${API_BASE_URL}/api/sensors/recent?count=${count}`
		);
	}

	// Get statistics for a specific sensor
	public async getSensorStatistics(
		sensorId: number
	): Promise<ApiSensorStatistics> {
		return this.fetchJson<ApiSensorStatistics>(
			`${API_BASE_URL}/api/sensors/${sensorId}/stats`
		);
	}
	...
	public async getHealth(): Promise<ApiHealthResponse> {
		return this.fetchJson<ApiHealthResponse>(
			`${API_BASE_URL}/api/sensors/health`
		);
	}

	// Performance monitoring
	public async getPerformance(): Promise<ApiPerformanceResponse> {
		return this.fetchJson<ApiPerformanceResponse>(
			`${API_BASE_URL}/api/sensors/performance`
		);
	}
	...
	public async resetPerformance(): Promise<{ message: string }> {
		const response = await fetch(
			`${API_BASE_URL}/api/sensors/performance/reset`,
			{
				method: "POST",
			}
		);
	...
	public transformApiStatistics(
		apiStats: ApiSensorStatistics
	): SensorStatistics {
		const sensorTypeMap: Record<string, SensorType> = {
			Temperature: SensorType.Temperature,
			Humidity: SensorType.Humidity,
	...
	public async getStatisticsBySensorType(): Promise<
		Record<SensorType, SensorStatistics>
	> {
	...
	public async getRecentReadingsBySensorType(
		count: number = 100
